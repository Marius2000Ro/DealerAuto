                                    ...
                                    foreach( $query_run as $row)
                                    {
                                        ?>
                                                <tr> 
                                                  ...
                                                  <td><?= $row['verified'];?></td>
                                                  <td><?= $row['blocked'];?></td>
                                                  <td><a href="users-deleted-restaureaza.php?id_user=<?= $row['id_user'];?>" class="btn btn-success">Anuleaza stergerea</td>
                                                  <?php if($_SESSION['auth_role'] != "1"){ ?>
                                                  <td>
                                                        <form action="actiuneButoane.php" method="POST">
                                                        <button type="submit" name="user_delete_definitiv" value="<?= $row['id_user'];?>" class="btn btn-danger" onclick="return confirm('Ești sigur că vrei să ștergi acest utilizator definitiv?');">Sterge definitiv</button></td>
                                                        </form>
                                                        <?php } ?>
                                                 </tr>
                                        <?php
